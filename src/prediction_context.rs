...
fn opt_eq(
    ...
    match arg {
        (Some(s), Some(o)) => Rc::ptr_eq(s, o) || *s == *o,
        (None, None) => true,
        _ => false,
    }
}
...
pub struct SingletonPredictionContext {
    cached_hash: i32,
    return_state: isize,
    parent_ctx: Option<Rc<PredictionContext>>,
}
...
thread_local! {
    pub static EMPTY_PREDICTION_CONTEXT: Rc<PredictionContext> =
        Rc::new(PredictionContext::new_empty());
}
...
//            | PredictionContext::Empty { cached_hash, .. }
    ...
    #[inline(always)]
    pub fn alloc(mut self) -> Rc<PredictionContext> {
        self.calc_hash();
        Rc::new(self)
    }
    ...
    pub fn merge(
        ...
        let r = match (a.deref(), b.deref()) {
            ...
            (sa, sb) => {
                if root_is_wildcard {
                    if sa.is_empty() {
                        return EMPTY_PREDICTION_CONTEXT.with(|x| x.clone());
                    }
                    if sb.is_empty() {
                        return EMPTY_PREDICTION_CONTEXT.with(|x| x.clone());
                    }
    ...
    fn merge_singletons(
        ...
        Self::merge_root(a, b, root_is_wildcard).unwrap_or_else(|| {
            if a.return_state == b.return_state {
                ...
                if Rc::ptr_eq(&parent, a.parent_ctx.as_ref().unwrap()) {
                    Singleton(a.clone())
                } else if Rc::ptr_eq(&parent, b.parent_ctx.as_ref().unwrap()) {
                    Singleton(b.clone())
                } else {
    ...
    fn merge_root(
        ...
        if root_is_wildcard {
            if a.is_empty() || b.is_empty() {
                return Some(EMPTY_PREDICTION_CONTEXT.with(|x| x.clone()));
            }
        } else {
            if a.is_empty() && b.is_empty() {
                return Some(EMPTY_PREDICTION_CONTEXT.with(|x| x.clone()));
    ...
    fn combine_common_parents(array: &mut ArrayPredictionContext) {
        let mut uniq_parents =
            HashMap::<Option<Rc<PredictionContext>>, Option<Rc<PredictionContext>>>::new();
...
pub struct PredictionContextCache {
    //todo test dashmap
    cache: RefCell<HashMap<Rc<PredictionContext>, Rc<PredictionContext>, MurmurHasherBuilder>>,
}
...
impl PredictionContextCache {
    ...
    pub fn new() -> PredictionContextCache {
        PredictionContextCache {
            cache: RefCell::new(HashMap::with_hasher(MurmurHasherBuilder {})),
        }
    }
    ...
    pub fn get_shared_context(
        ...
        if let Some(old) = self.cache.borrow().get(context) {
            return old.clone();
        }
